...
# base line
SQL = 'select b.retweet_count, b.content, a.follower, a.friend, a.favorite, a.entryCount from enjo_basedata as b left join aas_twitter_com.aas_twitter_com_%s as a on b.name = a.authorId left join all_users as au on au.name = b.name where topic = "spirits" and retweet_id = "0" and a.authorId is NOT NULL and (b.retweet_count >= ' + str(BURST_LIMIT) +  ' or (b.retweet_count <= 2 and b.retweet_count <> 0)) order by b.retweet_count desc'
...
def main():
    ...
    # f.write('Burst,Hashtag,Mention,URL,Length,Reply,Follower,Follow,Favorite,Entry,Cluster,Degree,Betweenness,Closeness,Eigen,Pagerank,Hub,Authority\n')
    for aas in AAS:
